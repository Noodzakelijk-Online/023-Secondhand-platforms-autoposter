...
| TD-003 | High | Job processing | 18, 20 | Worker has claim-once behavior for due queued jobs, but lacks stale running-job recovery and database-specific locking drills. | Add stale-claim timeout/requeue policy and run concurrency tests against the production database engine. |

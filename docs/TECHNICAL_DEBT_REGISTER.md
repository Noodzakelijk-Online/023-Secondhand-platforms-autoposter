...
| --- | --- | --- | --- | --- | --- |
| TD-001 | High | Auth/session security | 6, 32 | Bearer-only auth and CSRF posture are documented and tested, but distributed rate limiting is not complete. | Add persistent/distributed login throttling before multi-instance production deployment. |
| TD-002 | High | Platform integrations | 16 | eBay official API/OAuth foundations are not implemented; the real credential checklist now exists. | Add OAuth config model, callback flow, token storage strategy, and sandbox-only tests before any production automation. |
...
| TD-004 | Medium | Data deletion/privacy | 30 | Export/import/delete now write sanitized audit events, but there is no retention policy or admin/operator review UI. | Define audit retention rules and add an operator review path before release. |
...
| TD-011 | Medium | Observability | 56, 73 | Request/job logging and JSON/text formatting now exist, but metrics are not defined. | Decide whether to expose Prometheus-style metrics, hosted-platform metrics only, or a simpler local diagnostics counter set. |

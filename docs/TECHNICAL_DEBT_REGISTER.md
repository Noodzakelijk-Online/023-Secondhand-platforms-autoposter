...
| TD-003 | Medium | Job processing | 18, 20 | Worker now uses PostgreSQL-oriented `SKIP LOCKED` due-job claims, a SQLite-compatible fallback, and stale-running recovery with tests, but live target-database concurrency verification is still incomplete. | Verify claim and recovery behavior against the target PostgreSQL database under concurrent workers. |
...
| TD-005 | Low | File storage | 9 | Local and S3-compatible image storage paths exist, but signed URL delivery, image transformations, and deployment-specific bucket policies are not implemented. | Add signed URL delivery, lifecycle/bucket policy evidence, or image transformations only if the deployment target requires them. |
| TD-006 | Medium | Frontend browser QA depth | 52, 54 | UI covers core flows, diagnostics, structured error banners, retry guidance, prepublish review, copy-ready mapped fields, focused validation recovery, and executed Chromium workflow evidence. Full keyboard, screen-reader, zoom, and cross-browser responsive evidence is still incomplete. | Run the remaining browser/accessibility checklist for keyboard, zoom, screen-reader, and cross-browser responsive coverage. |
...
| TD-012 | Medium | Accessibility launch QA | 70 | Automated static accessibility smoke checks, Chromium UI/UX debugging rounds, and responsive/browser matrix evidence exist, but keyboard, zoom, and screen-reader evidence is not captured. | Run the remaining accessibility checklist in `docs/BROWSER_ACCESSIBILITY_QA.md` and capture findings. |
...
| TD-014 | Low | Internationalization | 60 | The primary frontend shell has English/Dutch copy catalogs, but dynamic API messages, diagnostics payloads, marketplace catalogs, and deeper locale formatting remain English-first. | Localize dynamic server messages and marketplace catalogs only when product language requirements require them. |
| TD-015 | Low | Release evidence | 70, 84, 85, 87, 88 | Local and fresh-clone verification reports plus a structured release evidence record exist, but browser evidence and deployment-specific launch evidence are not complete. | Complete `docs/RELEASE_EVIDENCE_RECORD.md` with production-target launch evidence when environment details are available. |

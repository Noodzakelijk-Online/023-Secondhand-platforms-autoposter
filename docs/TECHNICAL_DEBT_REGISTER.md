...
| TD-004 | High | Data deletion/privacy | 30 | Export/import and self-service account deletion exist, but privacy/audit event tracking is incomplete. | Add audit event model for export/import/delete actions and admin/operator review guidance. |
...
| TD-009 | Medium | Quality tooling | 44, 45, 71 | No formatter/linter/type-check configuration or CI workflow exists. | Add Ruff or equivalent, mypy/pyright decision, dependency audit, and GitHub Actions. |
...
| TD-011 | Medium | Observability | 56, 73 | Job logs exist, but app-level structured logging, metrics, and operator runbook are incomplete. | Add logging config, health/diagnostics runbook, and metrics decision. |

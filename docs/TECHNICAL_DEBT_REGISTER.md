...
| TD-002 | High | Platform integrations | 16, 68 | eBay official API/OAuth readiness is documented in the credential checklist and platform metadata, and raw secrets are rejected from account metadata, but no OAuth flow, Sell API client, or secret-manager reference model exists. | Add OAuth config model, callback flow, token reference strategy, and sandbox-only tests before any production automation. |
| TD-003 | High | Job processing | 18, 20 | Worker has claim-once behavior and stale running-job requeue, but lacks database-specific locking drills against the production engine. | Run concurrency tests against the production database engine and tune timeout/rate limits from real operations. |
...
| TD-012 | Medium | Accessibility/browser QA | 52, 53, 54 | Static accessibility audit exists for landmarks, labels, button names, image alt coverage, live region presence, and positive tabindex, but there is no browser/a11y audit report. | Add browser E2E, contrast checks, keyboard traversal, and assistive-technology smoke checks. |

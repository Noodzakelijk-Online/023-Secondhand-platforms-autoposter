...
.venv/bin/python scripts/audit_static_ui.py
.venv/bin/python -m pytest tests/test_static_ui_audit.py
...
- Static UI accessibility checks now guard against missing landmarks, labels, button names, image alt coverage, live region removal, and positive tabindex, but do not replace browser E2E, contrast, keyboard, or assistive-technology review.

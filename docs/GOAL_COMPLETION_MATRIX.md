...
| 103. Official provider credential readiness | Partial | `GET /api/platforms` now exposes official API status, credential requirements, and automation blockers; the UI displays those limits. Real eBay OAuth/Sell API publishing remains blocked on provider account approval, OAuth configuration, sandbox/production credentials, and secret storage. |
| 104. Static UI accessibility baseline | Partial | `scripts/audit_static_ui.py` and `tests/test_static_ui_audit.py` check landmarks, labels, button names, image alt coverage, live region presence, and positive-tabindex regressions. Browser E2E, contrast, responsive screenshots, keyboard traversal, and assistive-technology signoff remain open. |
